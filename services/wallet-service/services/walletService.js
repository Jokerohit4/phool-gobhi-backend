...
export async function purchaseSubscriptionWithWallet(customerId, gymId, planType, { coinCatalogItemKey } = {}) {
  ...
  // partnerShare is computed off the full, undiscounted price — a coin
  // discount is Phool Gobhi's own acquisition spend out of its commission
  // margin, never the partner's. It's the plan-lifetime ceiling, not a
  // one-time payment — the partner is credited partnerShare/days per
  // completed visit instead (see bookingCommissionFields/completeBooking in
  // booking-service), so this customer's own already-collected revenue is
  // what funds every visit (including any later gift-day make-up visits) and
  // unused days are simply never paid out. Still upsert the partner's wallet
  // row now so the first per-visit credit never fails on a missing wallet
  // (the bug that used to strand a purchase in PROCESSING when a partner had
  // no wallet yet).
